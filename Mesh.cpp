...
std::string wcharToString(const wchar_t* wcharStr)
{
	...
	WideCharToMultiByte(CP_UTF8, 0, wcharStr, -1, &strTo[0], size_needed, NULL, NULL);
	return strTo;
}
...
Mesh::Mesh(const wchar_t* full_path) : Resource(full_path)
{
	...
	std::string input_file = wcharToString(full_path);//std::wstring_convert<std::codecvt_utf8<wchar_t>>().to_bytes(full_path);
